...
const Todos = ({ todos, deleteTodo }) => {
  const todoList = todos.length ? (
    todos.map((todo) => {
      let color = "##ffffffff" // Default white color
      const today = new Date();
      const dueDate = new Date(todo.due);    

      console.log(`Todo: ${todo.content}, Due Date: ${dueDate}, Today: ${today}`);


      if (dueDate < today) {
        color = "#ffcccc" // Light red color for overdue tasks
      }
      else {
        color = "ccffcc" // Light green color for tasks not overdue
      }

      console.log(`Color for '${todo.content}': ${color}`);
